<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- x-data lives on the outer grid, not just the customer field, so the
                 "previous orders" panel on the right can share the same selectedId/
                 pastOrders state without sitting inside the order form's flow. --}}
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6"
                    ...
                    <div class="bg-white overflow-hidden shadow-sm rounded-lg p-6">
                        ...
                        <form method="POST" action="{{ route('orders.store') }}">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="relative">
                                    {{-- name/autocomplete deliberately avoid "name"/"email" so Chrome's --}}
                                    {{-- account-autofill heuristics don't try to fill in a Google address --}}
                                    <input type="text"
                                           x-model="query"
                                           ...
                                           data-1p-ignore
                                           data-lpignore="true"
                                           placeholder="Zoek een klant (naam of bedrijf)..."
                            ...
                            <div class="mb-4"
                                 x-data="{
                                     ...
                                     // Fills the rows from a past order (see the 'Herbestel deze'
                                     // button on the right) instead of submitting anything — the
                                     // order is only actually placed once 'Bestelling plaatsen' is
                                     // clicked below, so it can still be reviewed or adjusted first.
                                     loadFromPastOrder(pastItems) {
                                         this.items = pastItems.map(function (i) {
                                             return {
                                                 product_id: i.product_id ?? '',
                                                 product: i.product ?? '',
                                                 quantity: i.quantity ?? 1,
                                     ...
                                     // Live total, recalculated on every keystroke — see the display below.
                                     total() {
                                 ...
                                 @repeat-order.window="loadFromPastOrder($event.detail.items)">
                                    ...
                                    <div class="grid grid-cols-12 gap-2 mb-2 items-start">
                                        ...
                                        <div class="col-span-6 relative">
                                            <input type="text"
                                                   ...
                                                   class="w-full border-2 border-gray-300 rounded-md shadow-sm text-sm px-3 py-2.5 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                                        ...
                                        <div class="col-span-2">
                                            <input type="number" :name="`items[${index}][quantity]`" x-model="item.quantity"
                                                   ...
                                                   class="w-full border-2 border-gray-300 rounded-md shadow-sm text-sm px-3 py-2.5 font-medium focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                                        </div>
                                ...
                                {{-- Running total for the order being built, updates as quantity/price change --}}
                                <div class="mt-3 pt-3 border-t border-gray-200 flex items-center justify-end gap-2">
                                    <span class="text-sm text-gray-600">Totaal:</span>
                                    <span class="text-lg font-semibold text-gray-900" x-text="'€ ' + total().toFixed(2).replace('.', ',')"></span>
                                </div>
                            </div>
                ...
                {{-- Right: previous orders for the selected customer — reordering lives
                     here instead of a "Herbestellen" button on the whole orders list.
                     Its own card on the side so it never shifts the form on the left
                     while you're still filling it in. --}}
                <div class="lg:col-span-1" x-show="selectedId" x-cloak style="display: none;">
                    <div class="bg-white overflow-hidden shadow-sm rounded-lg p-6 lg:sticky lg:top-6">
                        <p class="text-sm font-medium text-gray-700 mb-3">Vorige bestellingen van deze klant</p>
                        ...
                        <ul x-show="!loadingOrders && pastOrders.length > 0" class="space-y-2">
                            <template x-for="pastOrder in pastOrders" :key="pastOrder.id">
                                <li class="text-xs bg-gray-50 rounded px-3 py-2">
                                    <p class="truncate">
                                        <span x-text="pastOrder.created_at"></span> —
                                        <span x-text="pastOrder.items_summary"></span>
                                    </p>
                                    <div class="flex items-center justify-between mt-1">
                                        <span>€ <span x-text="pastOrder.total"></span></span>
                                        {{-- Only fills the product rows on the left — nothing is
                                             submitted until 'Bestelling plaatsen' is clicked there. --}}
                                        <button type="button"
                                                @click="$dispatch('repeat-order', { items: pastOrder.items })"
